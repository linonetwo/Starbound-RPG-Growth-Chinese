...
export function keyPathInObject(obj: Object | string[], keys: string[], parentPath: string = ''): Patch[] {
  let keyPaths: Patch[] = [];
  // 看看是不是源文件中的 patch 文件
  if (isArray(obj)) {
    obj.forEach((item, index) => {
      if (typeof item === 'string') {
        keyPaths.push({ op: 'replace', value: item, path: `${parentPath}/${index}` });
        return;
      }
    ...
    for (const key in obj) {
      ...
      if (
        ...
        const isDatabaseID =
          keyMaybeDatabaseID.includes(key) &&
          ((keyToJudgeDatabaseID: string[]).some(id => id in obj) || parentPath.length === 0);
        if (!isDatabaseID) {
          keyPaths.push({ op: 'replace', value: obj[key], path: `${parentPath}/${key}` });
        }
      }
      // 检查这个字段是不是有子字段
      const isObjectArray = Array.isArray(obj[key]) && obj[key].length > 0 && isPlainObject(obj[key][0]);
      const isTextArray = Array.isArray(obj[key]) && obj[key].length > 0 && keysNeedTranslation.includes(key);
      if (!stopWordsForPath.includes(key) && (isPlainObject(obj[key]) || isObjectArray || isTextArray)) {
...
export async function sanitizeJSON(filePath: string) {
  ...
  // 修复多行字符串
  let result = replace(rawJSONString, /"[a-zA-Z_]+"[\n ]*:[\n ]*"[^"]+?"/g, (badMultilineLineString: string) => {
